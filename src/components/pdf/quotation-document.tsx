...
import type { CompanyProfile, Quotation } from "@/types";
...
/**
 * Pixel-accurate, print/PDF-ready A4 quotation.
 *
 * Styling uses fixed RGB Tailwind colors (slate/blue) — NOT theme CSS
 * variables — so html2canvas captures it identically in light or dark mode.
 * Width is fixed to 794px (A4 @ 96dpi). The parent controls scaling for
 * on-screen preview; `id="print-area"` is targeted by the print stylesheet.
 */
export const QuotationDocument = React.forwardRef<
  HTMLDivElement,
  QuotationDocumentProps
>(({ quotation, company, currencySymbol = "₹" }, ref) => {
  const { customer, items, totals, charges } = quotation;
        ...
        {totals.chargesTotal > 0 ? (
          <section className="mt-4 text-xs text-slate-500">
            ...
            {[
              ...
              ["Installation", charges.installationCharge],
              ["Visiting", charges.visitingCharge],
              ["Transport", charges.transportationCharge],
              ["Other", charges.otherCharges],
            ]
              .filter(([, v]) => (v as number) > 0)
              .map(([label, v]) => `${label}: ${formatCurrency(v as number, sym)}`)
              .join("  •  ")}
          </section>
        ) : null}
